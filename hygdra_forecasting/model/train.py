...
# Training Loop
def train_model(model:nn.Module, dataloader:DataLoader, val_dataloader:DataLoader=None, epochs:int=100, learning_rate:float=0.001, save_epoch=True, lrfn=CosineWarmup().lrfn):
    """
    ...
    for epoch in range(epochs):
        ...
        if val_dataloader :
            ...
            if val_loss < best_val_loss:
                ...
                if save_epoch:
                    save(model.state_dict(), 'weight/best_model.pt')


        if epoch % 40 == 0 and save_epoch:
            save(model.state_dict(), f'weight/epoch-{epoch}_loss-{round(running_loss / len(dataloader), 3)}.pt')
...
if __name__ == '__main__':
    ...
    tickers= ["INTC", "SQ", "BA", "PFE", "PYPL", "SBUX", "DIS", "NFLX", 'GOOG', "NVDA", "JNJ", "META", "GOOGL", "AAPL", "MSFT", "BTC-EUR", "CRO-EUR", "ETH-USD", "CRO-USD", "INJ-USD", "BTC-USD", "BNB-USD", "XRP-USD", "ADA-USD", "SOL-USD"]
    tickers_val = ['"AMZN"', "AMD", "ETH-EUR", "COST"]
    ...
    model.load_state_dict(load('weight/epoch-300_loss-0.12902073532342911.pt', weights_only=True))
    model = train_model(model, dataloader, dataloader_val, epochs=400, learning_rate=0.02, lrfn=CosineWarmup(0.02, 400).lrfn)
    save(model.state_dict(), 'weight/template_state.pt')
